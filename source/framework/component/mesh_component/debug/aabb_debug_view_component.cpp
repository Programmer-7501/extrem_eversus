...
#include "aabb_debug_view_component.h"
...
bool AABBDebugViewComponent::m_IsUseFlag = true;

void AABBDebugViewComponent::SetIsUseFlag(bool flag)
{
	m_IsUseFlag = flag;
}

AABBDebugViewComponent::AABBDebugViewComponent(Actor * owner)
	: MeshComponent(owner)
...
void AABBDebugViewComponent::Draw() const
{
	// nullチェック
	...
	//マトリクス設定
	Conv_XM::Matrix4x4f mtxT, mtxS, mtxW;
...
bool AABBDebugViewComponent::LoadData()
{
...
bool AABBDebugViewComponent::CreateMesh()
{
	...
	vertex[0].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[1].Position = Conv_XM::Vector3f(0.5f, 0.5f, -0.5f);
	vertex[1].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[2].Position = Conv_XM::Vector3f(-0.5f, -0.5f, -0.5f);
	vertex[2].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[3].Position = Conv_XM::Vector3f(0.5f, -0.5f, -0.5f);
	vertex[3].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[4].Position = Conv_XM::Vector3f(-0.5f, 0.5f, 0.5f);
	vertex[4].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[5].Position = Conv_XM::Vector3f(0.5f, 0.5f, 0.5f);
	vertex[5].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[6].Position = Conv_XM::Vector3f(-0.5f, -0.5f, 0.5f);
	vertex[6].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);

	vertex[7].Position = Conv_XM::Vector3f(0.5f, -0.5f, 0.5f);
	vertex[7].Color = Conv_XM::Vector4f(0.0f, 0.0f, 1.0f, 1.0f);
...
bool AABBDebugViewComponent::GetDeviceContext()
{
	// デバイスコンテキストの取得
...
bool AABBDebugViewComponent::GetShader()
{
	// 頂点シェーダーの取得
