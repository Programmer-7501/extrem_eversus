...
void Enemy::LoadModel()
{
	...
	m_AnimationModel = animationModelManager.Load(EnemyData::k_EnemyAnimationModelFileName);
}

void Enemy::UnloadModel()
{
	AnimationModelManager& animationModelManager = AnimationModelManager::GetInstance();
	animationModelManager.Unload(EnemyData::k_EnemyAnimationModelFileName);
}
...
Enemy::Enemy()
	...
	, m_EnemyNumber(ENEMY001)
{
	m_ActorType = Actor::ACTORTYPE_ENEMY;
}
...
void Enemy::SetActive(bool flag)
{
	...
	if (m_Active == false)
	{
		...
		// コストを減らす
		m_CostManager->DecreaseEnemyCost(m_MyCost);
	}
}

void Enemy::SetIsMoveFlag(bool flag)
{
	m_StateComponent->SetIsMoveFlag(flag);
}

void Enemy::LoadActor()
{
	// モビルスーツの読み込み処理
...
void Enemy::InitActor()
{
	...
	{
		Logger::GetInstance().SetLog("Enemy::InitActor currentSceneがnullptr");
		return;
	}
	SceneCollisionManager* sceneCollisionManager = currentScene->GetSceneCollisionManager();
	if (sceneCollisionManager == nullptr)
	{
		Logger::GetInstance().SetLog("Enemy::InitActor sceneCollisionManagerがnullptr");
		return;
	}
	//　コリジョンのセット
	if (m_EnemyNumber == ENEMY001)
	{
		sceneCollisionManager->SetMobileSuitData(SceneCollisionManager::ENEMY001, this, m_RigidBodyComponent, m_OBBCollisionComponent, m_AABBCollisionComponent);
	}
	else
	{
		sceneCollisionManager->SetMobileSuitData(SceneCollisionManager::ENEMY002, this, m_RigidBodyComponent, m_OBBCollisionComponent, m_AABBCollisionComponent);
	}
...
void Enemy::ProcessInputActor()
{
	...
	if (m_TargetNum == 0)
	{
		...
		// ロックオン距離かどうかを判定
		if (distanceSq > EnemyData::k_DefaultLockOnDistanceSq)
		{
			m_IsLockOn = false;
		}
		else
...
void Enemy::ResetBeamRifleCoolTime()
{
	m_IsBeamRifleShot = false;
	m_BeamRifleCountFrame = 0;
}

void Enemy::CollisionDetected(Actor * coleObj)
{
	...
	{
	case Actor::ACTORTYPE_PLAYER_BULLET:
	...
	case Actor::ACTORTYPE_CANNON:
		...
		m_DownNumCoolTime = 0;
		break;
	case Actor::ACTORTYPE_BEAMSABER_NKAKU001:
		if (m_ColedType == ACTORTYPE_BEAMSABER_NKAKU001)
		{
			break;
		}
		m_LifeComponent->PlusLife(-DamageData::k_BeamRifleDamage);
		...
		m_StateComponent->ChangeMobileSuitState("MobileSuitStateBendBackWard");
		m_DownNumCoolTime = 0;
		m_ColedType = ACTORTYPE_BEAMSABER_NKAKU001;
		break;
	case Actor::ACTORTYPE_BEAMSABER_NKAKU002:
		if (m_ColedType == ACTORTYPE_BEAMSABER_NKAKU002)
		{
			break;
		}
		m_LifeComponent->PlusLife(-DamageData::k_BeamRifleDamage);
		...
		m_DownNumCoolTime = 0;
		break;
	case Actor::ACTORTYPE_BEAMSABER_NKAKU003:
		if (m_ColedType == ACTORTYPE_BEAMSABER_NKAKU003)
		{
			break;
		}
		m_LifeComponent->PlusLife(-DamageData::k_BeamRifleDamage);
