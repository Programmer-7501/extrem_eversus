...
void MakeNodeScene::LoadData()
{
	...
	// フェード開始
	m_Fade->SetFade(Fade::FADE_OUT, MakeNodeSceneData::k_FadeOutTime);
	...
	DebugNumberBillBoardComponent::LoadNumberTexture();

	WeightedGraphMaker::LoadModel();
	otherActorManager->CreateActor<WeightedGraphMaker>();
...
void MakeNodeScene::UnloadData()
{
	DebugNumberBillBoardComponent::UnloadNumberTexture();

	WeightedGraphMaker::UnloadModel();
...
void MakeNodeScene::SetData()
{
}

void MakeNodeScene::UpdateScene()
{
	// ImGui入力
...
void MakeNodeScene::CreateSceneFunction()
{
	// シーンレンダー作成
	if (m_SceneRenderManager == nullptr)
	{
...
void MakeNodeScene::UnloadSceneFunction()
{
